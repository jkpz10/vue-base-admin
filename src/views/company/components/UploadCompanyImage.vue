<template>
  <v-sheet
    v-bind="$attrs"
    class="pos-relative file-upload"
  ...
  >
    ...
    <v-tooltip right>
      <template v-slot:activator="{ on, attrs }">
        <span class="input-container" v-bind="attrs" v-on="on">
          <!-- hide-input -->
          <v-file-input
            :value="file"
...
import { APP_ERROR, REQUEST_UPLOAD_IMAGE } from '@/store/modules/app' // REQUEST_UPLOAD_IMAGE
...
export default {
  ...
  props: {
    ...
    type: {
      type: String, // select type = 'avatar', 'so2', 'temperature', 'companyLogo', 'facilityLogo'
      default: '',
    },
  },
  data: () => ({
